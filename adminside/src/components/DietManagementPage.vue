          ...
          <div v-else class="table-container">
            <el-table :data="historicalOrders" style="width: 100%">
              ...
              <el-table-column
                ...
                width="180"
              ></el-table-column>
              <!-- Add more columns for admin view if needed, e.g., payment details -->
...
onMounted(() => {
  if (isLoggedIn.value) {
    fetchAllHistoricalOrders(); // Fetch ALL historical orders
  }
});

// Function to fetch ALL historical orders
const fetchAllHistoricalOrders = async () => {
  ...
  try {
    ...
    // Assuming you have a backend API endpoint to get ALL orders
    const response = await axios.get(
      "http://localhost:3060/api/homecare2/all-orders", // New endpoint to get all orders
      {
        headers: {
